...
\subsection{ImageNet-1k vs ImageNet-R}
Per ottenere dei valori statisticamente significativi, abbiamo ripetuto la classificazione per cinque volte, in modo da ottenere più misurazioni delle metriche e calcolare gli intervalli di confidenza. I risultati ottenuti sono presentati in tabella \ref{tab:base_drift_metrics}. In particolare, la colonna Sig. indica la significatività statistica, secondo il test t Welch, del delta tra le due misure ottenute.
\begin{table}[htbp]
\centering
\begin{tabular}{lcccc}
\hline
\textbf{Metric} & \textbf{ImageNet-1k (mean)} & \textbf{ImageNet-R (mean)} & \textbf{Delta (mean $\pm$ CI)} & \textbf{Sig.} \\
\hline
...
\hline
\end{tabular}
\caption{Confronto tra le metriche di performance del modello}
\label{tab:base_drift_metrics}
\end{table}
Come si può notare dalla tabella, tutte le metriche registrare calano significativamente quando il modello si trova a classificare i dati affetti da drift. Infatti, ImageNet-R presenta un drift sia sulle feature visive (cinque feature su sei presentano drift) sia sulle feature del modello. In particolare, l'accuracy, su cui ci siamo concentrati durante l'analisi di ImageNet-C, cala drasticamente, registrando un decremento del $53.9\%$.
\subsection{ImageNet-1k vs ImageNet-C}
...
\begin{tabular}{lccc}
\hline
\textbf{Corruption Type} & \textbf{AUC} & \textbf{Slope} & $\mathbf{R^2}$ \\
\hline
...
\hline
\end{tabular}
\caption{Metriche di performance per la classificazione con corruzione di tipo blur}
\label{tab:blur_perfomance}
\end{table}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/categories/blur_accuracy.png}
  \caption{Valore dell'accuracy al variare della severità per corruzioni di tipo blur}
  \label{fig:blur_accuracy}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/degradation/blur_accuracy_degradation.png}
  \caption{Degradazione dell'accuracy al variare della severità per corruzioni di tipo blur}
  \label{fig:blur_degradation}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/robustness/blur_accuracy_robustness.png}
  \caption{Valore di robustness al variare della severità per corruzioni di tipo blur}
  \label{fig:blur_robustness}
\end{figure}
Come si può notare dal grafico in figura \ref{fig:blur_degradation}, la degradazione dell'accuracy in generale cresce al variare della severity. Ciò è coerente con l'andamento dell'accuracy riportata in figura \ref{fig:blur_accuracy}. In particolare, il coefficiente $R^2$ calcolato per glass\_blur indica che la degradazione cresce meno linearmente rispetto agli altri tipi di blur. Infatti, si registra un aumento repentino della degradazione dalla severity 3. Inoltre, al livello di severity 5, il valore di accuracy per glass\_blur raggiunge un valore inferiore a $0.1$, infatti presenta una AUC della robustezza molto bassa. 
D'altro canto, il coefficiente $R^2$ calcolato per zoom\_blur indica che la degradazione delle performance cresce linearmente con il livello di severity, indicando una correlazione tra degradazione e severità per questo tipo di sporcatura. Possiamo inoltre notare che nonostante non risulti un drift per i primi due livelli di severity per motion\_blur, come rilevato dal test MMD esposto in tabella \ref{tab:mmd_drift_report_imagenet_c_blur}, l'accuracy del modello cala comunque di molto nel secondo livello di severity. Infine, glass\_blur è il tipo di sporcatura che più degrada le performance del modello al livello di severity 4 e 5, ed è anche il tipo di blur che ha la maggiore distanza nel test MMD negli ultimi due livelli.
\subsubsection{Digital}
\begin{table}[htbp]
\centering
...
\hline
\end{tabular}
\caption{Metriche di performance per la classificazione con corruzione di tipo digital}
\label{tab:digital_corruption_metrics}
\end{table}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/categories/digital_accuracy.png}
  \caption{Valore dell'accuracy al variare della severità per corruzioni di tipo digital}
  \label{fig:digital_accuracy}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/degradation/digital_accuracy_degradation.png}
  \caption{Degradazione dell'accuracy al variare della severità per corruzioni di tipo digital}
  \label{fig:digital_degradation}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/robustness/digital_accuracy_robustness.png}
  \caption{Valore di robustness al variare della severità per corruzioni di tipo digital}
  \label{fig:digital_robustness}
\end{figure}
Come si può notare dal grafico in figura \ref{fig:digital_accuracy}, l'accuracy decresce con il livello di severità della corruzione eccezion fatta per elastic\_transform, la quale presenta un breve incremento dell'accuracy passando dal livello due al livello tre di severità. I risultati ottenuti si riflettono nei valori di robustezza del modello, presentati in figura \ref{fig:digital_degradation}. In particolare, la degradazione delle performance non segue affatto un andamento lineare per contrast e elastic\_transform, mentre per JPEG\_compression e pixelate sono più vicini ad un andamento lineare.
Contrast mantiene dei valori di accuracy relativamente alti ai primi tre livelli di severity, che secondo il test MMD presentato in tabella \ref{tab:mmd_drift_report_imagenet_c_digital} non presentano drift nelle feature estratte dal modello. Similmente, jpeg\_compression non presenta drift a nessun livello di severity, e mantiene un'accuracy relativamente alta ai primi tre livelli di severity. Elastic\_trasform ha un calo di accuracy molto alto al livello di severity 5, che secondo il test MMD ha una distanza molto alta rispetto agli tipi di sporcatura digitale (0.0740). 
\subsubsection{Noise}
\begin{table}[htbp]
\centering
...
\hline
\end{tabular}
\caption{Metriche di performance per la classificazione con corruzione di tipo noise}
\label{tab:noise_corruption_metrics}
\end{table}
\begin{figure}[htbp]
  \centering
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/degradation/noise_accuracy_degradation.png}
  \caption{Degradazione dell'accuracy al variare della severità per corruzioni di tipo noise}
  \label{fig:noise_degradation}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/robustness/noise_accuracy_robustness.png}
  \caption{Valore di robustness al variare della severità per corruzioni di tipo noise}
  \label{fig:noise_robustness}
\end{figure}
Come si può notare dalla tabella \ref{tab:noise_corruption_metrics} e dalle figure \ref{fig:noise_accuracy} e \ref{fig:noise_degradation}, il calo di performance e l'aumento della degradazione seguono un andamento molti vicino ad un andamento lineare. Le performance del modello per questa classe di sporcatura calano molto al variare della severity, arrivando addirittura a un valore di quasi $0.1$ di accuratezza a livello di severità 5. I valori AUC riflettono questo calo, arrivando nel caso di impulse\_noise ad un valore inferiore a $0.5$.
Gaussian\_noise e shot\_noise non presentano drift nel primo livello di severity, dove si riesce a mantenere un'accuracy relativamente alta. Ad eccezione di questi, è presente drift per tutti i tipi di noise agli altri livelli, presentando un calo rapido e marcato dall'accuracy, raggiungendo valori molto bassi al livello di severity 5, dove il test MMD ha rilevato una distanza alta per tutti i tipi di noise.
\subsubsection{Weather}
\begin{table}[htbp]
\centering
\begin{tabular}{lccc}
...
\hline
\end{tabular}
\caption{Metriche di performance per la classificazione con corruzione di tipo weather.}
\label{tab:weather_corruption_metrics}
\end{table}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/categories/weather_accuracy.png}
  \caption{Valore dell'accuracy al variare della severità per corruzioni di tipo weather}
  \label{fig:weather_accuracy}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/degradation/weather_accuracy_degradation.png}
  \caption{Degradazione dell'accuracy al variare della severità per corruzioni di tipo weather}
  \label{fig:weather_degradation}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width = \textwidth]{images/plot_metrics/robustness/weather_accuracy_robustness.png}
  \caption{Valore di robustness al variare della severità per corruzioni di tipo weather}
  \label{fig:weather_robustness}
\end{figure}
Come si può notare dai valori in tabella \ref{tab:weather_corruption_metrics} e dalle figure \ref{fig:weather_accuracy} e \ref{fig:weather_degradation}, l'accuratezza registrata dal modello decresce per ogni tipo sporcatura e per ogni livello di severity. Tuttavia, il decremento dell'accuracy è decisamente meno marcato per brightness, su cui il modello riesce a mantenere performance relativamente elevate rispetto agli altri tipi di sporcatura, mantenendo anche un AUC della robustezza molto elevata. Inoltre, la degradazione delle performance per brightness segue un andamento quasi lineare ma con un'inclinazione bassa. Inoltre, nonostante il calo dell'accuracy, fog mantiene comunque un'AUC della robustezza moderatamente elevata, indicando quindi resistenza maggiore a questo tipo di sporcatura.
Possiamo osservare che secondo il test MMD illustrato in tabella \ref{tab:mmd_drift_report_imagenet_c_weather}, brightness e fog presentano un drift solo al quinto livello di severity, mentre frost e snow presentano drift a partire dal secondo livello di severity. Ciò si riflette nei valori di accuracy, dove frost e snow presentano un calo più marcato rispetto a brightness e fog. Pur presentando brightness e fog un drift solo al quinto livello di severity, il modello riesce a mantenere performance migliori per brightness rispetto a fog, questo può essere giustificato dal fatto che fog ha una distanza del test MMD maggiore rispetto a brightness, indicando quindi un drift più marcato nelle feature estratte dal modello per fog rispetto a brightness.