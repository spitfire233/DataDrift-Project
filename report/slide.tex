...
\author[Besozzi, Consonni, Zen]{Daniele Besozzi, Andrea Consonni, Elena Zen}
...
\begin{frame}{Il problema del Data Drift}
    \begin{itemize}
        \item \textbf{Assunzione classica (i.i.d.)}: Nel machine learning si assume che i dati di test e di addestramento provengano dalla stessa distribuzione.
...
\begin{frame}{ImageNet-1k (La Baseline)}
    \begin{itemize}
        ...
        \item \textbf{Nel progetto}: Utilizziamo il validation set di 50.000 immagini come benchmark \textbf{in-distribution} per stabilire le prestazioni ottimali del modello.
    \end{itemize}
\end{frame}
...
\begin{frame}{Il problema della degradazione e le ResNet}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item Apprendimento residuale: imparare la differenza $\mathcal{F}(x)$ invece della mappa completa.
...
\begin{frame}{Pipeline di Testing e Preprocessing}
    \begin{itemize}
        \item \textbf{Librerie}: Sviluppo in PyTorch, elaborazione dataset tramite HuggingFace e Arrow per l'efficienza. Evidenze di drift tramite \texttt{Evidently}.
        \item \textbf{Filtro Dataset}: Per il test su ImageNet-R, ImageNet-1k è stato filtrato mantenendo solo le 200 classi in comune, per garantire un confronto paritario.
        ...
        \item \textbf{Metriche}: Calcolate Accuracy, Balanced Accuracy, Precision, Recall, F1, MCC e Cohen's Kappa su 5 ripetizioni con \textit{augmentation stocastica} per stimare gli Intervalli di Confidenza (Test $t$ di Welch).
    \end{itemize}
\end{frame}
...
\begin{frame}{Le due facce del Drift}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item Misurato tramite Test \textbf{Empirical MMD} (Maximum Mean Discrepancy, drift se p-value $< 0.05$).
        \end{itemize}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Drift sulle Feature della Rete (MMD)}
    Anche internamente al modello, ImageNet-R genera un forte \textbf{Drift Latente}.
    \vspace{0.5cm}
    \begin{table}[]
        \centering
        \begin{tabular}{llll}
...
\begin{frame}{ImageNet-C: Drift Visivo - Categoria Blur}
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \textbf{Distribuzioni Blur} (\textit{es. Glass Blur / Defocus}):
            \begin{itemize}
        ...
        \begin{column}{0.6\textwidth}
            \centering
            \includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{images/metrics/ImageNet-C/distributions_by_corruption/glass_blur.png}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{ImageNet-C: Drift Visivo - Categoria Noise}
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \textbf{Distribuzioni Noise} (\textit{es. Gaussian / Shot Noise}):
            \begin{itemize}
        ...
        \begin{column}{0.6\textwidth}
            \centering
            \includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{images/metrics/ImageNet-C/distributions_by_corruption/gaussian_noise.png}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{ImageNet-C: Drift Visivo - Categoria Weather}
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \textbf{Distribuzioni Weather} (\textit{es. Fog / Snow}):
            \begin{itemize}
                \item La nebbia appiattisce il contrasto comprimendo le distribuzioni verso un grigio uniforme.
                \item La neve altera la luminosità e crea picchi anomali di nitidezza grazie ai fiocchi definiti.
            \end{itemize}
        \end{column}
        \begin{column}{0.6\textwidth}
            \centering
            \includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{images/metrics/ImageNet-C/distributions_by_corruption/fog.png}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{ImageNet-C: Drift Visivo - Categoria Digital}
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \textbf{Distribuzioni Digital} (\textit{es. Contrast / Elastic Transform}):
            \begin{itemize}
                \item La manipolazione diretta del contrasto sposta radicalmente la metrica RMS contrast.
                \item Le medie dei canali colore rimangono incredibilmente stabili nonostante la forte alterazione visiva.
            \end{itemize}
        \end{column}
        \begin{column}{0.6\textwidth}
            \centering
            \includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{images/metrics/ImageNet-C/distributions_by_corruption/contrast.png}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Drift Feature Rete (MMD) vs Drift Visivo su ImageNet-C}
    ...
    \begin{itemize}
        ...
        \item \textbf{Reti Neurali}: Il test MMD \textbf{non} rileva drift sulle feature di \texttt{avgpool} per i primi livelli di severità per corruzioni come \textit{Contrast}, \textit{Motion Blur} o \textit{Gaussian Noise}. Per \textit{JPEG\_compression}, non rileva drift a nessuna severità.
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                ...
                \item \textbf{Glass Blur} ha un crollo drastico non lineare (AUC bassissima: 0.369), portando l'accuratezza ben sotto il 10\% a severità 5.
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                ...
                \item Ai primissimi livelli (1-2) il modello regge parzialmente (coerentemente con l'assenza temporanea di drift MMD).
                \item A severità 5 l'accuratezza crolla irrimediabilmente a $\sim 10\%$.
                \item \textbf{Impulse noise} risulta il peggiore della categoria (AUC 0.482).
            \end{itemize}
        \end{column}
        \begin{column}{0.6\textwidth}
            \centering
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                ...
                \item \textbf{JPEG} e \textbf{Contrast} mantengono accuratezze relativamente buone (AUC $>$ 0.73), il che si allinea perfettamente con l'assenza di drift MMD sulle feature interne a basse/medie severità.
                \item \textbf{Elastic Transform} fa eccezione: dopo un andamento stabile crolla precipitosamente a severità 5.
            \end{itemize}
        \end{column}
        \begin{column}{0.6\textwidth}
            \centering
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{itemize}
                ...
                \item \textbf{Brightness} non impatta quasi per nulla il modello, mantenendo un'accuratezza altissima (AUC 0.899).
                \item Anche \textbf{Fog} (nebbia) si mantiene moderatamente robusta.
                \item Al contrario, \textbf{Frost} e \textbf{Snow} distruggono le performance (AUC $\sim$ 0.50-0.57), probabilmente perché introducono artefatti fisici complessi (cristalli e occlusione) anziché soli cambi cromatici.
            \end{itemize}
        \end{column}
        \begin{column}{0.6\textwidth}
            \centering
...
\begin{frame}{Sviluppi Futuri}
    ...
    \begin{itemize}
        \item \textbf{Drift Temporale}: Analizzare ImageNet-V2, dataset campionato successivamente ma con logiche identiche a ImageNet originale, per testare la riproducibilità temporale e il natural shift.
        \item \textbf{Altri Shift}: Ripetere l'analisi introducendo \textit{Prior Probability Shift} (classi sbilanciate artificialmente) o \textit{Concept Shift} (attacchi avversariali attivi).
        \item \textbf{Architetture Alternative}: Valutare se modelli concettualmente diversi, come i \textbf{Vision Transformers (ViT)}, mostrino una robustezza superiore o diversa rispetto alle CNN in presenza dello stesso drift.
    \end{itemize}
\end{frame}
